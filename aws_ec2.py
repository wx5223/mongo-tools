...
class AwsEc2(object):
    ...
    def __init__(self, aws_key_id=None, aws_secret_key=None):
        if aws_key_id and aws_secret_key:
            ...
            self.connection = session.resource("ec2")
        else:
            try:
    ...
    def launch_instance(self,
        ...
        for block_device in block_devices:
            ...
            bdm["Ebs"] = {
                ...
                "VolumeSize": block_devices[block_device]["size"]}
            if "type" in block_devices[block_device]:
                bdm["Ebs"]["VolumeType"] = block_devices[block_device]["type"]
                if "iops" in block_devices[block_device]:
                    bdm["Ebs"]["Iops"] = block_devices[block_device]["iops"]
            bdms.append(bdm)
        if bdms:
...
def main():
    ...
    aws_options = optparse.OptionGroup(parser, "AWS options")
    ...
    aws_options.add_option("--awsKeyId",
                           dest="aws_key_id",
                           default=None,
                           help="AWS access key id.")

    aws_options.add_option("--awsSecretKey",
                           dest="aws_secret_key",
                           default=None,
    ...
    volume_types = ["default", "standard", "io1", "gp2", "sc1", "st1"]
    create_options.add_option("--blockDevice",
                              dest="block_devices",
                              metavar="DEVICE-NAME VOLUME-SIZE-GB VOLUME-TYPE IOPS",
                              action="append",
                              default=[],
                              nargs=4,
                              help="EBS device name, volume size in GiB, volume type and IOPS."
                                   " More than one device can be attached, by specifying"
                                   " this option more than once. Choose volume type"
                                   " from {}. When specifying 'default' as volume type "
                                   " then the IOPS value will be ignored. The device will"
                                   " be deleted on termination of the"
                                   " instance.".format(volume_types))
    ...
    parser.add_option_group(aws_options)
    parser.add_option_group(control_options)
    parser.add_option_group(create_options)

    (options, args) = parser.parse_args()

    aws_ec2 = AwsEc2(aws_key_id=options.aws_key_id, aws_secret_key=options.aws_secret_key)
    ...
    if options.mode == "create":
        ...
        block_devices = {}
        for (device_name, volume_size, volume_type, iops) in options.block_devices:
            try:
                volume_size = int(volume_size)
            except ValueError:
                parser.print_help()
                parser.error("Block device volume size must be an integer")
            try:
                iops = int(iops)
            except ValueError:
                parser.print_help()
                parser.error("Block device IOPS must be an integer")
            if volume_type not in volume_types:
                parser.error("Block device volume type must be one of {}".format(volume_types))
            block_devices[device_name] = {}
            block_devices[device_name]["size"] = int(volume_size)
            if volume_type != "default":
                block_devices[device_name]["type"] = volume_type
                if iops:
                    block_devices[device_name]["iops"] = int(iops)
